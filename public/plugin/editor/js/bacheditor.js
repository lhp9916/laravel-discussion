...
<!-- end 列表、引用 -->\
    ...
    function toggleBold(editor) {
        ...
        if (stat.bold) {
            if (curPoint || cm.getRange({
                    ...
                    ch: startPoint.ch - 2
                }, startPoint) !== '**' || cm.getRange(endPoint, {line: endPoint.line, ch: endPoint.ch + 2}) !== '**') {    // 没有选中或选中的不全
                return;
            } else {
    ...
    function toggleItalic(editor) {
        ...
        if (stat.italic) {
            if (curPoint || cm.getRange({
                    line: startPoint.line,
                    ch: startPoint.ch - 1
                }, startPoint) !== '*' || cm.getRange(endPoint, {line: endPoint.line, ch: endPoint.ch + 1}) !== '*') {    // 没有选中或选中的不全
                return;
            } else {
                text = cm.getSelection();
                startPoint.ch -= 1;
    ...
    function toggleTitle(editor) {
        ...
        if (!curPoint && cm.getRange({
                line: startPoint.line,
                ch: startPoint.ch - 2
            }, startPoint) === '##' && cm.getRange(endPoint, {line: endPoint.line, ch: endPoint.ch + 2}) === '##') {
